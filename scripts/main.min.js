 ...
 */
 ;(function($) {
	...
	if (isIOS) {
		...
		if($header.length > 0){
			$('body').on('focus', 'input', function () {
				$header.addClass("posr");
				$h80.hide();
			}).on('focusout', 'input', function () {
				$header.removeClass("posr");
				$h80.show();
			});
		}
	}
...
;(function(win,doc,$){
				...
				for(var i=0;i<arr.length;i++){
					...
					if(index == 0){
						rs += '<a href="'+ baseurl+o.goods_id +'" class="item">'+
							...
							'<div class="txt"><p class="money">￥'+ o.price +'</p><p class="cont bot">接单'+ o.service_num +'</p></div>'+
						'</a>';
					}else{
	...
	win.fn_16_17 = function(){
		var tempNum = $("#hiddenInfor").data("num");
		new SwitchOption({
...
;(function(win,doc,$){
	...
	win.fn_h_2 = function(){
	    function init() {
	        ...
	        function validateNumber(obj,val){
	            ...
	            if( !val ){
	                tips = obj.data("tips") + "不能为空!";
	            } else if( !/^[1-9]*[1-9][0-9]*$/.test(val) ){
	            	tips = obj.data("tips") + "只能是正整数!";
	            }
	            return tips;
	        }
	        ...
	        $("#n2_register").on(CLICK_EVENT,function(){
	            ...
	            if( !$that.hasClass("noTap") ){
	                ...
	                tips += validateNumber($workYear,workYearVal);
	                if( /!/.test(tips) ){
	                    $.popUp.messages( {"innerHTML":tips,"callBack":function(){
	                        setTimeout(function(){
	                            $that.removeClass("noTap");
	                ...
	                tips += validate($description,descriptionVal);
	                if( /!/.test(tips) ){
	                    $.popUp.messages( {"innerHTML":tips,"callBack":function(){
	                        setTimeout(function(){
	                            $that.removeClass("noTap");
	                ...
	                $.ajax({
						...
						success: function(data){
							$that.removeClass("noTap");
							if(data.desc !== "" && data.desc){alert(data.desc);}
							if(data.code == 1){
								location.href = data.redirect_uri;
							} 
						},
						...
						rs += '<div class="h_btm">'+
	                        '<a href="'+baseurl+o.id+'" class="n2_lb_box">'+
	                            ...
	                            '<span class="user_info">'+
	                                '<span class="imgBox"><img src="'+o.head_img+'" alt="" class="nimg"/></span>'+
	                                '<span class="info">'+
				...
				'callFn': function(rs,cb){
					...
					if(rs.code==1){
						...
						if(data){
							...
							$.each($str.find(".imgBox"),function(i,val){
								var $that = $(this);
								autoResizeImage($that.find(".nimg"),$that);
							});
	...
	win.fn_h_13 = function(){
	    function init() {
	        ...
	        var $sdTxt = $("#n2_dp_txt");
	        // 获取当前时间
	        var twoFix = function(n){
				return n > 9 ? n : ('0' + n);
			}
			var year = now.getFullYear();
			var dbMonth = twoFix(now.getMonth() + 1);
			var dbDay = twoFix(now.getDate());
			$sdTxt.html(dbMonth+"."+dbDay);
		    $datetime.val(year+"/"+dbMonth+"/"+dbDay);
	        ...
	        $sd.mobiscroll().date({
	            ...
	            stepMinute: 60,
	            onSelect: function (valueText, inst) {
		            $sdTxt.html(valueText.split("/").splice(1,2).join("."));
		            $datetime.val(valueText);
		        }
	        });
...
;(function(win,doc,$){
	...
	win.fn_3 = function(){
		...
		setCollect("c_shou");
		var $imgBoxP = $(".person");
	    autoResizeImage($imgBoxP.find("img"),$imgBoxP);
		shareFriend({});
	}
	win.fn_6 = function(){
		getCollect("c_shou");
	...
	*/
	win.fn_18 = function(){
	    function init() {
	    	...
	    	getCollect("c_shou");
			setCollect("c_shou");
			// $("#iSlider-effect-wrapper").height($("#iSlider-effect-wrapper").width()*0.75);
			...
			var picList = window.globalpicList;
	                ...
	                $.ajax({
						...
						success: function(data){
							if(data.code == 1){
								...
								$.popUp.messages( {"innerHTML":data.desc,"callBack":function(){
			                        setTimeout(function(){
			                            $that.removeClass("noTap");
			                        },3000);
			                    }} );
	...
	win.fn_h_20 = function(){
		...
		if(gTag){
		gTag = false;
		$("p").bind(CLICK_EVENT,function(){
			var radioId = $(this).find("label").attr("name");
			$('label').removeAttr('class') && $(this).find("label").attr('class', 'checked');
	...
	win.fn_h_21 = function(){
		$("#n_order").bind(CLICK_EVENT,function(){
			...
			if( !subtn.hasClass("noTap") ){
				...
				//3.新密码输入不一致
				else if(newPass !== checkPass){tips = "您输入的确认密码和新密码不一致!";}
				if(tips != ""){
					$.popUp.messages( {"innerHTML":tips,"callBack":function(){
						setTimeout(function(){
					...
					success: function(msg){
						subtn.removeClass("noTap");
						//alert(msg.desc);
						if(msg.desc !== "" && msg.desc){alert(msg.desc);}
						if(msg.code == 1){location.href = msg.redirect_uri;}
					},
					error:function(msg){
						subtn.removeClass("noTap");
						return;
