...
;(function(win,doc,$){
			...
			CyUtil.isFunction(fn) && fn.call(that, rs, function(pid,len){
				...
				if((arguments.length>1 && len<that.num) || !rs.nextid){
					if(that.isFirst){
						...
						that.btn.addClass('load_more_disable').html('没数据了亲~').show();
					}
					that.isFirst = false;
				} else {
					that.btn.show();
     ...
     * 自动裁剪图片
     * @param jQuery objImg
     * @param jQuery objImgBox
     */
	function autoResizeImage(objImg, objImgBox){
		if(!objImg.length || !objImgBox.length){ return;}
		var img = objImg[0];
		var maxWidth = objImgBox.width(),maxHeight = objImgBox.height();
		var width, height, maxRatio, imgRatio, wRatio, hRatio,nWidth,nHeight,offset,direction;
		var resizeImage = function(){
			...
			if(maxRatio  > imgRatio  ){
			    //是竖图
			    nWidth = maxWidth;
			    nHeight = height*wRatio;
			    offset = -(nHeight -maxHeight)/2;
			    objImgBox.css('background','none');
			    objImg.css({'width':nWidth + "px",'height':nHeight + "px", 'top':offset+"px", 'left':"0px"}).show();
			}else {
				//是横图
			    nHeight = maxHeight;
			    nWidth = width*hRatio;
			    offset = -(nWidth-maxWidth)/2;
			    objImgBox.css('background','none');
			    objImg.css({'width':nWidth + "px",'height':nHeight + "px", 'left':offset+"px", 'top':"0px"}).show();
			}
		}
	...
	win.autoResizeImage = autoResizeImage;
				...
				'callFn': function(rs,cb){
					...
					if(rs.code==1){
						...
						if(data){
							...
							$.each($str.find(".imgBox"),function(i,val){
								var $that = $(this);
								autoResizeImage($that.find(".img"),$that);
							});
		...
		var createHtml = function(arr, index){
			...
			if(arr){
				for(var i=0;i<arr.length;i++){
					...
					var fimg = 'http://side.video.sina.com.cn/weixin/themes/weixin/images/logo.png'
					if(o.userportrait){
						fimg = o.userportrait;
					}
					
					rs += '<a href="javascript:;" class="item">'+
						'<span class="imgBoxEqual_135"><img src="'+ fimg +'" alt="" class="img"/></span>'+
			...
			if(arr){
				for(var i=0;i<arr.length;i++){
					...
					rs += '<a href="'+ baseurl+o.uid +'" class="item">'+
						'<span class="imgBox"><img src="'+ o.portrait +'" alt="" class="img"/></span>'+
						'<div class="msg"><p class="name top">'+ o.fullname +'</p><p class="cnt bot">'+ o.level +'</p></div>'+
						'<div class="txt"><p class="money top">￥'+ o.price +'</p><p class="cont bot">接单'+ o.service_num +'</p></div>'+
					'</a>';
				}
		...
		$('#search span').bind(CLICK_EVENT, function(){
			if(bFlag){
				...
				if(!key && !searchInitFirst){
					$.popUp.messages({"innerHTML":'请输入化妆师姓名！'});
					return false;
				}
...
;(function(win,doc,$){
	...
	win.fn_6 = function(){
		...
		var $imgBox = $(".c_show");
		autoResizeImage($imgBox.find("img"),$imgBox);
		var evt = "onorientationchange" in window ? "orientationchange" : "resize";
		window.addEventListener(evt, function() {
		    autoResizeImage($imgBox.find("img"),$imgBox);
		}, false);

		var imgurl = $imgBox.find("img").attr('src');
			...
			'callFn': function(rs,cb){
				...
				if(rs.code==1){
					...
					if(data.list.length>0){
						...
						var $str=$(str);
						oBox.append($str);
						$.each($str.find(".c_cl"),function(i,val){
							var $that = $(this);
							autoResizeImage($that.find("img"),$that);
						});
	...
	win.fn_h_21 = function(){
		$("#n_order").bind(CLICK_EVENT,function(){
			...
			if( !subtn.hasClass("noTap") ){
				...
				if(oldPass === ""){tips = "旧密码不能为空！";}
				//2.新密码不能为空
				else if(newPass === ""){tips = "新密码不能为空！";}
				//3.新密码输入不一致
